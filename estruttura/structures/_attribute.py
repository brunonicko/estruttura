...
from ..base import Base, BaseImmutableMeta, BaseImmutable, BaseClass
...
class StructureAttributeMeta(BaseImmutableMeta):
    """Metaclass for :class:`StructureAttribute`."""


# noinspection PyAbstractClass
class StructureAttribute(
    six.with_metaclass(StructureAttributeMeta, BaseImmutable, ImmutableBasicData, Generic[T_co, RT])
):
    __slots__ = (
        "_owner",
        "_name",
...
AT_co = TypeVar("AT_co", bound=StructureAttribute, covariant=True)  # attribute type


class MutableStructureAttributeMeta(StructureAttributeMeta):
    pass


class MutableStructureAttribute(six.with_metaclass(MutableStructureAttributeMeta, StructureAttribute[T, RT])):
    __slots__ = ()
...
class AttributeMap(Base, SlottedHashable, SlottedMapping[str, AT_co]):
    ...
    def get_update_values(self, updates, instance=None):
        # type: (Mapping[str, Any], BaseClass | None) -> tuple[dict[str, Any], dict[str, Any]]

        # Compile update values.
        delegate_self = _DelegateSelf(self, instance)
...
class _DelegateSelf(Base):
    ...
    def __init__(self, attribute_map, instance=None):
        # type: (AttributeMap, BaseClass | None) -> None
        """
        :param attribute_map: Attribute map.
        :param instance: BaseClass instance that owns attributes.
        """
        internals = _DelegateSelfInternals(self, attribute_map, instance)
...
class _DelegateSelfInternals(Base):
    ...
    def __init__(self, iobj, attribute_map, instance):
        # type: (_DelegateSelf, AttributeMap, BaseClass | None) -> None
        """
        ...
        self.__dirty = set(attribute_map).difference(
            (list(zip(*(list(instance or [])))) or [[], []])[0]
        )  # type: set[str]
    ...
    def __set_new_value(self, name, value):
        ...
        try:
            if self.__instance is None:
                raise KeyError()
            old_value = self.__instance[name]
        except KeyError:
            old_value = DELETED
        ...
        for dependent in self.__attribute_map[name].recursive_dependents:
            ...
            try:
                if self.__instance is None:
                    raise KeyError()
                old_value = self.__instance[dependent.name]
            except KeyError:
    ...
    def instance(self):
        # type: () -> BaseClass | None
        """BaseClass instance that owns attributes."""
