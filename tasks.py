...
@task
def conform(c):
    c.run("isort estruttura tests ./docs/source/conf.py setup.py tasks.py -m 3 -l 88 --up --tc --lbt 0")
...
@task
def lint(c):
    c.run("isort estruttura tests ./docs/source/conf.py setup.py tasks.py -m 3 -l 88 --up --tc --lbt 0 --check-only")
...
def docs(c):
    ...
    os.mkdir(api_docs)
    c.run("sphinx-apidoc estruttura --separate --module-first --no-toc --force --output-dir {}".format(api_docs))
